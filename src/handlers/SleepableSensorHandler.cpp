...
// Constructor
// ─────────────────────────────────────────────
SleepableSensorHandler::SleepableSensorHandler(EventBus& bus, ManageHa& ha, ManageSleep& sleep, uint8_t pin)
    : _eventBus(bus), _ha(ha), _sleep(sleep), _pin(pin) {}

void SleepableSensorHandler::begin() {
  _sleep.registerParticipant("sleepable sensor", 1, *this);  // phase 1 — before mqtt and wifi
...
void SleepableSensorHandler::onSleep() {
  ...
  olog.info(TAG, "Sleep — shutting down sleepable sensor");
}