...
RelayHandler::RelayHandler(EventBus& bus, ManageHa& ha, uint8_t pin)
    : _eventBus(bus), _ha(ha), _pin(pin)
{}
...
void RelayHandler::begin() {
    ...
    _eventBus.subscribe(EventType::APP_MQTT_MESSAGE_RECEIVED,
        [this](EventType, const void* p) {
