...
// Constructor
// ─────────────────────────────────────────────
DS18B20Handler::DS18B20Handler(EventBus& bus, ManageHa& ha,
                               uint8_t pin, uint32_t interval)
    : _eventBus(bus)
    , _ha(ha)
    , _oneWire(pin)
    , _sensors(&_oneWire)
    , _interval(interval)
{
    _eventBus.subscribe(EventType::APP_MQTT_CLIENT_CONNECTED,
        [this](EventType, const void*) {
            _mqttReady = true;
            readAndPublish();   // immediate reading on every connect
        });

    _eventBus.subscribe(EventType::APP_MQTT_CLIENT_DISCONNECTED,
        [this](EventType, const void*) {
            _mqttReady = false;
        });
}
...
void DS18B20Handler::scanBus() {
    ...
    for (uint8_t i = 0; i < count; i++) {
       ...
       olog.info(TAG, "  bytes: %02x %02x %02x %02x %02x %02x %02x %02x",
                  s.address[0], s.address[1], s.address[2], s.address[3],
                  s.address[4], s.address[5], s.address[6], s.address[7]);
    }
}
...
void DS18B20Handler::readAndPublish() {
    ...
    for (const auto& sensor : _found) {
        float temp = DS18B20Config::USE_CELSIUS
        ...
        if (!isValidReading(temp)) {
            ...
            olog.warn(TAG, "Skipping %s — bad reading (%.1f)",
                      sensor.shortId.c_str(), temp);
...
void DS18B20Handler::publishDiscovery() {
    ...
    String stateTopic = StringUtils::joinTopic(
                            _ha.getBaseTopic(), "temperatures");
    ...
    for (const auto& sensor : _found) {
        ...
        olog.info(TAG, "Registered HA entity: %s → %s",
                  id.c_str(), stateTopic.c_str());
    }
}

String DS18B20Handler::addressToShortId(const DeviceAddress& addr) const {
    // bytes 3,4,5,6 — wider window for uniqueness across batches
    char buf[9];
